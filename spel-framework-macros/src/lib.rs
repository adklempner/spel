...
fn expand_lez_program(input: ItemMod, config: ProgramConfig) -> syn::Result<TokenStream2> {
    ...
    let main_fn = quote! {
        pub fn main() {
            ...
            // LEZ v0.2.2 requires every declared account in the output diff
            // (DeclaredAccountMissingFromOutput). Handlers may emit posts for
            // only a positional prefix of the declared accounts; pad the tail
            // with unmodified echoes — the claim check only fires when
            // pre != post, so echoed signers/read-only accounts pass. The
            // rc6-era filter that dropped default-owned accounts is gone.
            let mut post_states = post_states;
            for pre in pre_states_clone.iter().skip(post_states.len()) {
                post_states.push(nssa_core::program::AccountPostState::new(
                    pre.account.clone(),
                ));
            }
